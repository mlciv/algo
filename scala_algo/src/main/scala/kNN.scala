...
class KNN_Classifier(x: MatrixD, y: VectorI, fn: Array[String], k: Int, cn: Array[String],
  ...
  // degug flag
  private val MAX_DOUBLE = Double.PositiveInfinity
  // infinity
  private val topK = Array.ofDim[Tuple2[Int, Double]](knn)
  // top-knn nearest points (in reserve order)
...
object KNN_ClassifierTest extends App {
  val x = new MatrixD((6, 2), 1.0, 2.0, // data/feature matrix
    ...
    9.0, 8.0,
    8.0, 9.0)
  val y = VectorN(0, 0, 0, 1, 1, 1)
  // classification for each vector in x
  val fn = Array("x1", "x2")
